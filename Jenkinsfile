...
node {
    ...
    stage('Deploy'){
        ...
        withEnv(["ANSIBLE_HOST_KEY_CHECKING=False"]) {
          ansiblePlaybook credentialsId: 'varkeys-rhel-jenkins-password',
          installation: 'ansible 2.7.0',
          inventory: 'provision/inventory.ini',
          playbook: 'provision/playbook.yml',
          extras: "-vvv -e artifactPath=${artifactPath} -e artifactId=${artifactId} -e artifactVersion=${artifactVersion} -e packaging=${packaging}"
        }
    }
}
